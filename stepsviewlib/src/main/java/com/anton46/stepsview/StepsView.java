...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener {
    ...
    public static final int EMPTY_STEPS = -1;
    ...
    public StepsView setTextSize(int size) {
        this.textSize = size;
        return this;
    }
    ...
    public void drawView() {
        ...
        if (mCompletedPosition < -1 || mCompletedPosition > mLabels.length - 1) {
            throw new IndexOutOfBoundsException(String.format("Index : %s, Size : %s", mCompletedPosition, mLabels.length));
        }

        if (mLabelsLayout.getChildCount() > 0) {
            TextView firstView = (TextView) mLabelsLayout.getChildAt(0);
            if (textSize != firstView.getTextSize())
                drawLabels(true);
        }
    ...
    private void drawLabels(boolean clearList) {
